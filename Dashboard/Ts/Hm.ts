...
const an = document.querySelector(".announcement") as HTMLElement;
...
const res2:Promise<any> = jsonAlertFile();
...
const res3:Promise<any> = jsonAnnFile();
...
function resizefunction() {
  ...
  let hamburgermenudisplay:string = window.getComputedStyle(hamburgermenu).display;
  const dashboardoption = document.querySelector(".dashboardoption") as HTMLElement;
  let dashboardoptiondisplay:string = window.getComputedStyle(dashboardoption).display;
...
res2.then((alert) => {
  const len:number = alert.alert.length;
  const bg:string = "background-color:white;";
...
res3.then((ann) => {
  const ln:number = ann.announcement.length;
...
res1.then((card) => {
  const cardlen: number = card.Carddetails.length;
  for (let i: number = 0; i < cardlen; i++) {
    cards.innerHTML += `
